...
![ADR-022_S3_Object_to_REST_API_Protocol_Mapping_Translator](diagrams/ADR-022_S3_Object_to_REST_API_Protocol_Mapping_Translator.png)
...
The prefix-cache contract is explicit about namespace modality:

- **Unified Namespace** uses `/prefix-cache/unified/{namespace}/prefixes/{prefix_id}` for exact put/get/delete and `/prefix-cache/unified/{namespace}/search` or `/invalidate` for collection operations.
- **Dimensional Indexed Namespace** uses `/prefix-cache/dimensional/{namespace}/indexes/{index_id}/prefixes/{prefix_id}` for exact put/get/delete and `/prefix-cache/dimensional/{namespace}/indexes/{index_id}/search` or `/invalidate` for indexed collection operations.

`/prefix-cache/{namespace}` is not an exact-operation family. Exact operations require `prefix_id`; collection operations require an explicit search or invalidation payload.

## Context

Project Coherent Storage treats inference actors as clients of a governed cache/storage control plane rather than direct consumers of lower-layer storage internals. This ADR adds an access-layer translator that allows existing S3 tooling and direct REST clients to operate against the same logical object/key/value/vector namespace.
...
- metadata capture for request path, translation decisions, cache hits, cache misses, latency, object identity, and debugging; and
...
The paired translator report previously used ID-scoped prefix-cache routes while this ADR listed `/prefix-cache/{namespace}` as a family. This revision resolves that mismatch by adopting ADR-023 namespace modalities and reserving keyless collection routes only for search and predicate invalidation.
...
| `/prefix-cache/unified/{namespace}/prefixes/{prefix_id}` | Unified Namespace exact prefix-cache put/get/delete. |
| `/prefix-cache/unified/{namespace}/search` | Unified Namespace collection search. |
| `/prefix-cache/unified/{namespace}/invalidate` | Unified Namespace collection invalidation. |
| `/prefix-cache/dimensional/{namespace}/indexes/{index_id}/prefixes/{prefix_id}` | Dimensional Indexed Namespace exact prefix-cache put/get/delete. |
| `/prefix-cache/dimensional/{namespace}/indexes/{index_id}/search` | Dimensional Indexed Namespace indexed search. |
| `/prefix-cache/dimensional/{namespace}/indexes/{index_id}/invalidate` | Dimensional Indexed Namespace indexed invalidation. |
...
The meta service maps S3 wire operations into a normalized command envelope before calling REST.
...
The REST API response is translated back into an S3-shaped response for S3 clients, including status codes, headers, XML list/error payloads where needed, ETag handling, and S3-compatible metadata projection.
...
class CacheBackend:
    ...
    def put_prefix_cache(
        self,
        namespace: str,
        prefix_id: str,
        artifact: bytes,
        metadata: dict,
        namespace_mode: str = "unified",
...
## Namespace modality policy

ADR-023 owns namespace modality semantics. This translator must implement those semantics without exposing lower-layer infrastructure.

| Modality | Client-visible contract | Primary use |
| --- | --- | --- |
| Unified Namespace | `namespace` plus `prefix_id`; route authority hidden by Coherence-CE. | Simple global identity and compatibility. |
| Dimensional Indexed Namespace | `namespace`, `index_id`, `prefix_id`, and dimension descriptor. | Locality-aware scale-out/scale-up, bounded search, bounded invalidation, epoch-aware routing. |

The translator may accept a deprecated compatibility alias `/prefix-cache/{namespace}/{prefix_id}` only when explicitly configured. That alias must resolve to Unified Namespace semantics and must not appear as the preferred OpenAPI path.
...
- Scope prefix-cache invalidation by namespace modality, index, predicate, tenant policy, and locality epoch.
...
- OpenAPI schema documents object, KV, vector, prefix-cache, namespace modality, translation, health, and metrics endpoints.
...
- Exact prefix-cache put/get/delete routes require `prefix_id`.
- Search and invalidate are collection routes with explicit body schemas.
...
- Client generation has explicit routes for Unified Namespace and Dimensional Indexed Namespace operation.
...
- Dimensional indexing requires index lifecycle, epoch rotation, and telemetry governance.
...
- SDK helper that derives `index_id` from an authorized namespace descriptor.
...
- `adr/diagrams/ADR-022_S3_Object_to_REST_API_Protocol_Mapping_Translator.puml`
- `adr/diagrams/ADR-022_S3_Object_to_REST_API_Protocol_Mapping_Translator.png`
- `adr/diagrams/ADR-022_S3_Object_to_REST_API_Protocol_Mapping_Translator.svg`
